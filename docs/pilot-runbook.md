...
- eligible messages, shadow recommendations, automatic approvals, manual overrides, sampled
  false approvals, unnecessary quarantines, and resulting automation rate;
...
- known adversarial messages automatically approved: **0**;
...
The automation exit gate additionally requires at least 70% correct automatic approval among
eligible messages, no security-critical false approval, full review of the first 100 automatic
decisions and a random audit of at least 10% thereafter. A critical false approval disables
automation, adds a regression test, and restarts the affected observation window.
